...
tb2ConfigFile = et.parse('tb2Settings.xml')
# Find the root element from the file (in this case "environment")
root = tb2ConfigFile.getroot()
# Save the Robot Selected
tb2_namespace = root.findtext('TB2_ROS_NAMESPACE')
print(tb2_namespace)
...
def main():
    ...
    image_sub = rospy.Subscriber(tb2_namespace+"/tb1_image",Image,callback)
    odom_sub = rospy.Subscriber(tb2_namespace+'/odom',Odometry,callbackOdom)
    noteBatt_sub = rospy.Subscriber(tb2_namespace+"/laptop_charge/",
                                    SmartBatteryStatus,
                                    notebookBattery)
    kobukiBatt_sub = rospy.Subscriber(tb2_namespace+"/mobile_base/sensors/core",
                                      SensorState,
                                      kobukiBattery)
    rospy.spin()
